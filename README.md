...
This work now supports both a pure simulation engine and an optional loopback socket engine (`--engine socket`) for real transfer measurement. Its architecture mirrors the logical structure of packet-scheduling systems and is designed to be extended further for capstone/research use. The implemented extension set includes AIMD congestion modeling, packet-level DRR/HTB modules, DiffServ-style QoS classes, trace replay, a live web dashboard, and M/M/c + M/G/1 analytical validation. It reflects a sustained interest in the intersection of **networked systems**, **high-performance computing**, and **communication engineering**, and an ambition to pursue graduate-level study in these domains.
